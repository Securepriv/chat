...
export default function ChatClient() {
  ...
  async function enableNotifications() {
    ...
    if (Notification.permission === 'granted') {
      setNotificationsEnabled(true);
      setActionMessage('Notifications déjà activées.');
      return;
    }

    if (Notification.permission === 'denied') {
      setActionMessage('Notifications bloquées. Activez-les dans les paramètres du navigateur.');
      return;
    }

    const permission = await Notification.requestPermission();
    setNotificationsEnabled(permission === 'granted');
    setActionMessage(permission === 'granted' ? 'Notifications activées.' : 'Notifications non activées.');
  }
